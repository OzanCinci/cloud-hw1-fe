...
function Phones() {
    ...
    const handleDelete = (e,itemId) => {
        ...
        console.log(user)
        console.log(req);
        console.log('https://cloud-hw1-be.onrender.com/phones/' + itemId);
        // This is a simple POST request with a JSON body.
        fetch('https://cloud-hw1-be.onrender.com/phones/' + itemId, {
            method: 'DELETE', 
            headers: {
            'Content-Type': 'application/json',
            },
    ...
    const renderFields = (item, parentKey = '') => {    
      ...
      const handleUpdate = (selectedItem) => {
        ...
        nav("/update/phones/" + selectedItem._id);
        const btn = document.getElementById("close-button-phones");
        if (btn)
            btn.click();
    }
